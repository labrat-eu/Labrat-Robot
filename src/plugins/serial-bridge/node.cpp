...
void SerialBridgeNodePrivate::processMessage(u8 *buffer, std::size_t size) {
  ...
  if (!header->check()) {
    node.getLogger().warning() << "Received message with wrong header checksum.";
    return;
  }
  ...
  if (header->magic != HeaderWire::magic_check) {
    node.getLogger().critical() << "Received message with wrong magic byte.";
    return;
  }
  if (header->version_major != HeaderWire::version_major_check) {
    node.getLogger().critical() << "Received message of different major version number, discarding message.";
    return;
  }
  if (header->version_minor != HeaderWire::version_minor_check) {
    node.getLogger().warning() << "Received message of different minor version number.";
  }
  if (header->length != size - sizeof(HeaderWire)) {
    node.getLogger().critical() << "Datagram and header length mismatch, discarding message. (" << header->length << "/" << size - sizeof(HeaderWire) << ")";
    return;
  }
  ...
  switch (header->type) {
    case (HeaderWire::Type::payload): {
      ...
      if (!raw->data.check(header->length - sizeof(u16))) {
        node.getLogger().warning() << "Received message with wrong checksum.";
        return;
      }
    ...
    default: {
      node.getLogger().critical() << "Received unknown message type.";
    }
  }
}
...
void SerialBridgeNodePrivate::readPayloadMessage(const SerialBridgeNode::PayloadInfo &message) {
  ...
  if (iterator == sender.adapter.end()) {
    node.getLogger().debug() << "Received bridge message without adapter entry (remote topic hash: " << message.topic_hash << ").";
...
void SerialBridgeNodePrivate::readTopicInfoMessage(const TopicInfo &message) {
  ...
  if (iterator == sender.map.end()) {
    node.getLogger().warning() << "No registered handling implementation found (topic name: " << message.topic_name << ").";
    return;
  }

  if (iterator->second->getTopicInfo().type_name != message.type_name) {
    node.getLogger().warning() << "Local and remote type names do not match (" << iterator->second->getTopicInfo().type_name << "/" << message.type_name << ").";
    return;
  }
...
void SerialBridgeNodePrivate::readTopicRequestMessage(std::size_t topic_hash) {
  ...
  node.getLogger().debug() << "Requested topic hash receiver not found (topic hash: " << topic_hash << ").";
}
...
void SerialBridgeNodePrivate::writePayloadMessage(const SerialBridgeNode::PayloadInfo &message) {
  ...
  if (message.payload.size() > payload_size) {
    node.getLogger().critical() << "Maximum payload size exceeded.";
    return;
  }
...
void SerialBridgeNodePrivate::writeTopicInfoMessage(const TopicInfo &message) {
  if (message.topic_name.empty()) {
    node.getLogger().critical() << "The sent topic name must not be empty.";
    return;
  }
  if (message.type_name.empty()) {
    node.getLogger().critical() << "The sent type name must not be empty.";
    return;
  }
